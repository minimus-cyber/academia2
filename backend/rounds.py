...
async def run_round(round_id: int, theme_en: str) -> AsyncGenerator[dict, None]:
    ...
    for senior in seniores:
        review_user_prompt = (
            ...
            f"As Senior Reviewer, identify: "
            f"(1) the strongest interdisciplinary connections across departments, "
            f"(2) any gaps or contradictions that need addressing, "
            f"(3) your overall recommendation. "
            f"State APPROVED or REQUEST_REVISION with justification. Maximum 400 words."
        )
